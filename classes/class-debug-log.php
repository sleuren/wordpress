...
class Debug_Log {
	...
	public function __construct() {
		...
		$this->wp_config = new WP_Config_Transformer; // already in the Sleuren\Classes namespace, so, no need to repeat
	...
	public function get_status() {
		...
		if ( 'enabled' == $status ) {
			...
			return '<div id="sleuren-log-status" class="sleuren-log-status"><strong>' . esc_html__( 'Error Logging', 'sleuren' ) . '</strong>: ' . ucfirst( esc_html__( 'enabled', 'sleuren' ) ) . ' ' . esc_html__( 'on', 'sleuren' ) . ' ' . esc_html( $date_time ) . '</div>';

		} elseif ( 'disabled' == $status ) {

			return '<div id="sleuren-log-status" class="sleuren-log-status"><strong>' . esc_html__( 'Error Logging', 'sleuren' ) . '</strong>: ' . ucfirst( esc_html__( 'disabled', 'sleuren' ) ) . ' ' . esc_html__( 'on', 'sleuren' ) . ' ' . esc_html( $date_time ) . '</div>';
	...
	public function get_autorefresh_status() {
		...
		if ( 'enabled' == $autorefresh_status ) {
			...
			return '<div id="sleuren-autorefresh-status" class="sleuren-autorefresh-status"><strong>Auto-Refresh</strong>: ' . ucfirst( esc_html__( 'enabled', 'sleuren' ) ) . '</div>';

		} elseif ( 'disabled' == $autorefresh_status ) {

			return '<div id="sleuren-autorefresh-status" class="sleuren-autorefresh-status"><strong>Auto-Refresh</strong>: ' . ucfirst( esc_html__( 'disabled', 'sleuren' ) ) . '</div>';
	...
	public function toggle_debugging() {
		...
		if ( isset( $_REQUEST ) ) {
			...
			if ( 'disabled' == $log_info['status']  ) {
				...
				if ( $this->wp_config->exists( 'constant', 'WP_DEBUG_LOG' ) ) {
					...
					if ( is_bool( $wp_debug_log_const ) ) {
						...
						if ( is_file( WP_CONTENT_DIR . '/debug.log' ) ) {
                            ...
                            $default_log_content = file_get_contents( WP_CONTENT_DIR . '/debug.log' );
							file_put_contents( $log_file_path, $default_log_content );
						...
						if ( is_file( $wp_debug_log_const ) && ( $wp_debug_log_const != $log_file_path ) ) {
							$custom_debug_log_content = file_get_contents( $wp_debug_log_const );
							file_put_contents( $log_file_path, $custom_debug_log_content );
				...
				$this->wp_config->update( 'constant', 'WP_DEBUG_LOG', get_option( 'sleuren_file_path' ), $options );
				...
				foreach ( $errors_master_list as $error ) {
					...
					$entry = array( 
							...
							$error['type'], 
							$error['details'], 
							$localized_timestamp . '<br /><span class="sleuren-faint">(' . sprintf( _n( '%s occurrence logged', '%s occurrences logged', $occurrence_count, 'sleuren' ), number_format_i18n( $occurrence_count ) ) . ')<span>',
					);
				...
				$data = array(
					'status'	=> 'enabled',
					'copy'		=> $copy,
					'message' 	=> '<strong>' . esc_html__( 'Error Logging', 'sleuren' ) . '</strong>: ' . esc_html__( 'Enabled on', 'sleuren' ) . ' ' . esc_html( $date_time ),
					'entries'	=> $entries,
					'size'		=> $file_size,
				);
				...
				$data = array(
					'status'	=> 'disabled',
					'copy'		=> false,
					'message' 	=> '<strong>' . esc_html__( 'Error Logging', 'sleuren' ) . '</strong>: ' . esc_html__( 'Disabled on', 'sleuren' ) . ' ' . esc_html( $date_time ),
					'entries'	=> '',
					'size'		=> '',
				);
	...
	public function toggle_autorefresh() {
		...
		if ( isset( $_REQUEST ) ) {
			...
			if ( $autorefresh_status == 'disabled' ) {
		        ...
		        update_option( 'sleuren_autorefresh', 'enabled', false );

				$data = array(
					'status'	=> 'enabled',
					'message' 	=> '<strong>' . esc_html__( 'Auto-Refresh', 'sleuren' ) . '</strong>: ' . esc_html__( 'Enabled', 'sleuren' ),
				);
		        ...
		        update_option( 'sleuren_autorefresh', 'disabled', false );

				$data = array(
					'status'	=> 'disabled',
					'message' 	=> '<strong>' . esc_html__( 'Auto-Refresh', 'sleuren' ) . '</strong>: ' . esc_html__( 'Disabled', 'sleuren' ),
				);
	...
	public function get_processed_entries() {
		...
		foreach( $latest_lines as $line ) {
			...
			if ( array_key_exists('1', $line) ) {
				...
				if ( false !== strpos( $error, ABSPATH ) ) {
					...
					if ( ( false !== strpos( $error_file, '/wp-admin/' ) ) || 
						...
						$error_source = __( 'WordPress core', 'sleuren' );
					} elseif ( ( false !== strpos( $error_file, '/wp-content/themes/' ) ) ) {
						$error_source = __( 'Theme', 'sleuren' );
					} elseif ( ( false !== strpos( $error_file, '/wp-content/plugins/' ) ) ) {
						$error_source = __( 'Plugin', 'sleuren' );
					} else {
						$error_source = '';	
					}
			...
			if ( ( false !== strpos( $error, 'PHP Fatal' )) || ( false !== strpos( $error, 'FATAL' ) ) || ( false !== strpos( $error, 'E_ERROR' ) ) ) {
				...
				$error_type 	= __( 'PHP Warning', 'sleuren' );
				...
				$error_type 	= __( 'PHP Notice', 'sleuren' );
			...
			if ( ! empty( $error_source ) ) {
				if ( 'WordPress core' == $error_source ) {
					...
					$error_details = '<span class="error-details">' . $error_details . '</span><hr />' . $error_source . '<br />' . __( 'File', 'sleuren' ) . ': <a href="' . $file_viewer_url . '" target="_blank" class="error-source-link">' . $error_file_path . '<span class="dashicons dashicons-visibility offset-down"></span></a><br />' . __( 'Line', 'sleuren' ) . ': ' . $error_file_line;
					...
					if ( ! defined( 'DISALLOW_FILE_EDIT' ) || ( false === constant( 'DISALLOW_FILE_EDIT' ) ) ) {
						...
						$error_details = '<span class="error-details">' . $error_details . '</span><hr />' . $error_source . ': <a href="' . $error_source_theme_uri . '" target="_blank" class="error-source-link">' . $error_source_theme_name . '<span class="dashicons dashicons-external offset-up"></span></a><br />' . __( 'File', 'sleuren' ) . ': <a href="' . $file_viewer_url . '" target="_blank" class="error-source-link">' . $error_file_path . '<span class="dashicons dashicons-visibility offset-down"></span></a><br />' . __( 'Line', 'sleuren' ) . ': ' . $error_file_line;
					} 
					if ( defined( 'DISALLOW_FILE_EDIT' ) && ( true === constant( 'DISALLOW_FILE_EDIT' ) ) ) {
						$error_details = '<span class="error-details">' . $error_details . '</span><hr />' . $error_source . ': <a href="' . $error_source_theme_uri . '" target="_blank" class="error-source-link">' . $error_source_theme_name . '<span class="dashicons dashicons-external offset-up"></span></a><br />' . __( 'File', 'sleuren' ) . ': ' . $error_file_path . '<br />' . __( 'Line', 'sleuren' ) . ': ' . $error_file_line;
					}
					...
					if ( ! defined( 'DISALLOW_FILE_EDIT' ) || ( false === constant( 'DISALLOW_FILE_EDIT' ) ) ) {
						...
						$error_details = '<span class="error-details">' . $error_details . '</span><hr />' . $error_source . ': <a href="' . $error_source_plugin_uri . '" target="_blank" class="error-source-link">' . $error_source_plugin_name . '<span class="dashicons dashicons-external offset-up"></span></a><br />' . __( 'File', 'sleuren' ) . ': <a href="' . $file_viewer_url . '" target="_blank" class="error-source-link">' . $error_file_path . '<span class="dashicons dashicons-visibility offset-down"></span></a><br />' . __( 'Line', 'sleuren' ) . ': ' . $error_file_line;
					} 
					if ( defined( 'DISALLOW_FILE_EDIT' ) && ( true === constant( 'DISALLOW_FILE_EDIT' ) ) ) {
						$error_details = '<span class="error-details">' . $error_details . '</span><hr />' . $error_source . ': <a href="' . $error_source_plugin_uri . '" target="_blank" class="error-source-link">' . $error_source_plugin_name . '<span class="dashicons dashicons-external offset-up"></span></a><br />' . __( 'File', 'sleuren' ) . ': ' . $error_file_path . '<br />' . __( 'Line', 'sleuren' ) . ': ' . $error_file_line;
					}
				}
			}
	...
	public function get_entries_datatable() {
		...
		?>
		<div>
			<select id="errorTypeFilter" class="sleuren-error-type-filter">
				<option value=""><?php esc_html_e( 'All Error Types', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'PHP Fatal', 'sleuren' ); ?>"><?php esc_html_e( 'PHP Fatal', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'PHP Warning', 'sleuren' ); ?>"><?php esc_html_e( 'PHP Warning', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'PHP Notice', 'sleuren' ); ?>"><?php esc_html_e( 'PHP Notice', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'PHP Deprecated', 'sleuren' ); ?>"><?php esc_html_e( 'PHP Deprecated', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'PHP Parse', 'sleuren' ); ?>"><?php esc_html_e( 'PHP Parse', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'PHP Exception', 'sleuren' ); ?>"><?php esc_html_e( 'PHP Exception', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'Database', 'sleuren' ); ?>"><?php esc_html_e( 'Database', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'JavaScript', 'sleuren' ); ?>"><?php esc_html_e( 'JavaScript', 'sleuren' ); ?></option>
				<option value="<?php esc_attr_e( 'Other', 'sleuren' ); ?>"><?php esc_html_e( 'Other', 'sleuren' ); ?></option>				
			</select>
		</div>
		<table id="sleuren" class="wp-list-table widefat striped">
			<thead>
				<tr>
					<th class="sleuren-entry-no">#</th>
					<th class="sleuren-entry-type"><?php esc_html_e( 'Error Type', 'sleuren' ); ?></th>
					<th class="sleuren-entry-details"><?php esc_html_e( 'Details', 'sleuren' ); ?></th>
					<th class="sleuren-entry-datetime"><?php esc_html_e( 'Last Occurrence', 'sleuren' ); ?></th>
				</tr>
			</thead>
			<tbody>
		<?php
		...
		foreach ( $errors_master_list as $error ) {
			...
			<tr>
				<td class="sleuren-entry-no"><?php echo esc_html( $n ); ?></td>
				<td class="sleuren-entry-type"><?php echo esc_html( $error['type'] ); ?></td>
				<td class="sleuren-entry-details"><?php echo wp_kses( $error['details'], 'post' ); ?></td>
				<td class="sleuren-entry-datetime"><?php echo esc_html( $localized_timestamp ); ?><br /><span class="sleuren-faint">(<?php printf( _n( '%s occurrence logged', '%s occurrences logged', $occurrence_count, 'sleuren' ), number_format_i18n( $occurrence_count ) ); ?>)<span></td>
			</tr>
	...
	public function get_dashboard_widget_entries() {
		...
		<style>
			...
			#sleuren_widget.postbox .inside {
				margin: 0;
				padding: 0;
			}

			.sleuren-dashboard-widget-entry {
				padding: 12px;
			...
			.sleuren-dashboard-widget-entry:nth-child(odd) {
				background-color: #f6f7f7;
			}

			.sleuren-dashboard-widget-entry-message a.error-source-link {
			    color: #50575e;
			    text-decoration: none;
			}

			.sleuren-dashboard-widget-entry-message a.error-source-link span {
			    position: relative;
			    margin-left: 2px;
			    color: #777;
			    font-size: 18px;
			...
			.sleuren-dashboard-widget-entry-message a.error-source-link span.offset-up {
			    top: -1px;
			}

			.sleuren-dashboard-widget-entry-message a.error-source-link span.offset-down {
			    top: 1px;
			}

			.sleuren-dashboard-widget-entry-message a.error-source-link:hover {
			    color: #2271b1;
			    text-decoration: underline;
			}

			sleuren .sleuren-entry-details a.error-source-link:hover span {
			    color: #2271b1;
			    text-decoration: none;
			}

			.sleuren-dashboard-widget-entry-meta {
				display: flex;
			}

			.sleuren-dashboard-widget-entry-type {
				margin-right: 4px;
				font-weight: 600;
			}

			.sleuren-dashboard-widget-footer {
				display: flex;
		...
		<div class="sleuren-dashboard-widget-entries">
		<?php
		...
		foreach ( $errors_master_list as $error ) {
			...
			if ( $n <= $entries_to_show ) {
				...
				?>
					<div class="sleuren-dashboard-widget-entry">
						<div class="sleuren-dashboard-widget-entry-meta">
							<div class="sleuren-dashboard-widget-entry-type">
								<?php echo esc_html( $error['type'] ); ?>
							</div>
							<div class="sleuren-dashboard-widget-entry-datetime">
								| <?php echo esc_html( $localized_timestamp ); ?>
							</div>
						</div>
						<div class="sleuren-dashboard-widget-entry-message">
		...
		<div class="sleuren-dashboard-widget-footer">
			<div class="sleuren-dashboard-widget-logging-status">
				<?php echo $this->get_status(); ?>
			</div>
			<a href="<?php echo get_dashboard_url(); ?>tools.php?page=sleuren" class="button"><?php esc_html_e( 'Go to Sleuren', 'sleuren' ); ?></a>
		</div>
		<?php
	...
	public function log_js_errors() {
		...
		if ( isset( $request['message'] ) && isset( $request['script'] ) && isset( $request['lineNo'] ) && isset( $request['columnNo'] ) && ! empty( $request['nonce'] ) && wp_verify_nonce( $request['nonce'], SLE_SLUG ) ) {
